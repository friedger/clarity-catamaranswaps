...
(define-map swaps uint {amount-buy: uint, buyer: principal, amount: uint, seller: (optional principal), when: uint, open: bool, ft-seller: principal, fees: principal})
...
(define-private (mia-transfer-to (amount uint) (to principal) (memo (buff 34)))
  (contract-call? 'SP466FNC0P7JWTNM2R9T199QRZN1MYEDTAR0KP27.miamicoin-token transfer amount tx-sender to (some memo)))

;; helper function to transfer ft to a principal with memo
...
;; create a swap between mia and fungible token
(define-public (create-swap (amount-buy uint) (amount-sell uint) (seller (optional principal)) (ft-sell <fungible-token>) (fees <fees-trait>))
  (let ((id (var-get next-id)))
    (asserts! (map-insert swaps id
      {amount-buy: amount-buy, buyer: tx-sender, amount-sell: amount, seller: ft-sender,
         when: block-height, open: true, ft-seller: (contract-of ft-sell), fees: (contract-of fees)}) ERR_INVALID_ID)
    (var-set next-id (+ id u1))
    (try! (contract-call? fees hold-fees amount-buy amount-sell ft-sell))
    (match (mia-transfer-to amount-buy (as-contract tx-sender) 0x636174616d6172616e2073776170)
      success (ok id)
...
(define-public (cancel (id uint) (fees <fees-trait>))
  (let ((swap (unwrap! (map-get? swaps id) ERR_INVALID_ID))
    ...
    (amount-sell (get amount-sell swap)))
      ...
      (try! (contract-call? fees release-fees amount-buy amount-sell ft-sell))
      (match (as-contract (mia-transfer-to
                amount-buy (get buyer swap)
    ...
    (seller (default-to tx-sender (get seller swap))))
      ...
      (match (ft-transfer-to
        ...
        success-sell (begin
            ...
            (match (as-contract (mia-transfer-to
                (get amount-buy swap) stx-receiver
                0x636174616d6172616e2073776170))
