...
(define-map swaps uint {mia: uint, banana-sender: principal, nft-id: uint, nft-sender: (optional principal), when: uint, open: bool, nft: principal, fees: principal})
...
(define-public (create-swap (mia uint) (nft-id uint) (nft-sender (optional principal)) (nft <non-fungible-token>) (fees <fees-trait>))
  (let ((id (var-get next-id)))
    (asserts! (map-insert swaps id
      {mia: mia, banana-sender: tx-sender, nft-id: nft-id, nft-sender: nft-sender,
...
(define-public (submit-swap
  ...
  (let ((swap (unwrap! (map-get? swaps id) ERR_INVALID_ID))
    ...
    (banana-receiver (default-to tx-sender (get nft-sender swap))))
      ...
      (asserts! (is-eq (contract-of fees) (get fees swap)) ERR_INVALID_FEES_TRAIT)
      (asserts! (map-set swaps id (merge swap {open: false})) ERR_NATIVE_FAILURE)
      (asserts! (is-eq tx-sender banana-receiver) ERR_INVALID_RECEIVER)
