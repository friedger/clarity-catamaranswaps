;; @description swap xbtc for btc using catamaran swap
;; USE WITH CARE due to limitation of this contract and Stacks 2.0
;; @version 1

(define-constant expiry u100)
(define-map swaps uint {sats: uint, btc-receiver: (buff 40), amount: uint, xbtc-receiver: (optional principal), xbtc-sender: principal, when: uint, open: bool})
...
(define-private (xbtc-transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (contract-call? 'SP3DX3H4FEYZJZ586MFBS25ZW3HZDMEW92260R2PR.Wrapped-Bitcoin transfer amount sender recipient memo))

;; create a swap between btc and xbtc
(define-public (create-swap (sats uint) (btc-receiver (buff 40)) (amount uint) (xbtc-receiver (optional principal)) )
  (let ((id (var-get next-id)))
    (asserts! (map-insert swaps id
      {sats: sats, btc-receiver: btc-receiver, amount: amount, xbtc-receiver: xbtc-receiver,
        xbtc-sender: tx-sender, when: block-height, open: true}) ERR_INVALID_ID)
    (var-set next-id (+ id u1))
    (match (xbtc-transfer amount tx-sender (as-contract tx-sender) (some 0x636174616d6172616e2073776170))
      success (ok id)
      error (err (* error u1000)))))


(define-public (set-xbtc-receiver (id uint))
  (let ((swap (unwrap! (map-get? swaps id) ERR_INVALID_ID)))
    (if (is-none (get xbtc-receiver swap))
      (begin
        (asserts! (map-set swaps id (merge swap {xbtc-receiver: (some tx-sender)})) ERR_NATIVE_FAILURE)
        (ok true))
      ERR_ALREADY_DONE)))
...
(define-public (cancel (id uint))
  (let ((swap (unwrap! (map-get? swaps id) ERR_INVALID_ID)))
    ...
    (as-contract (xbtc-transfer (get amount swap) tx-sender (get xbtc-sender swap) (some 0x72657665727420636174616d6172616e2073776170)))))
          ...
          (match (get out (unwrap! (get-out-value tx (get btc-receiver swap)) ERR_NATIVE_FAILURE))
            out (if (>= (get value out) (get sats swap))
              (begin
                    ...
                    (as-contract (xbtc-transfer (get amount swap) tx-sender (unwrap! (get xbtc-receiver swap) ERR_NO_FT_RECEIVER) (some 0x636174616d6172616e2073776170))))
