...
import { transfer } from "./client/usda.ts";

const contractName = "usda-nft-swap";
const assetSuffix = "usda-token::usda";

Clarinet.test({
  name: "User can cancel usda-nft swap after 100 blocks",
  async fn(chain: Chain, accounts: Map<string, Account>) {
    const deployer = accounts.get("deployer")!;
    const wallet_1 = accounts.get("wallet_1")!;
    const wallet_2 = accounts.get("wallet_2")!;
    const usdaAmount = 1_000_000;
    const nftId = 1;
    const swapId = 0;
    let block = chain.mineBlock([
      transfer(usdaAmount * 2, deployer, wallet_1, deployer),
    ]);
