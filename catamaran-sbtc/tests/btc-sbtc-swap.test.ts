...
describe("User can cancel btc-sbtc swap", () => {
  test("that Alice can create a swap with Bob and cancel it after", async () => {
    ...
    expect(tooEarly.value).toBe(btcSbtcSwap.constants.ERR_FORBIDDEN.value);
    ...
    // try cancel by alice after already canceled
    const cancel = txErr(btcSbtcSwap.cancel(requestId), alice);
    expect(cancel.value).toBe(btcSbtcSwap.constants.ERR_ALREADY_DONE.value);
  });
  ...
  test("that Alice can't cancel the swap with Bob after Bob registered btc tx", async () => {
    ...
    expect(submissionByAlice.value).toBe(
      btcSbtcSwap.constants.ERR_FORBIDDEN.value
    );
