...
(define-constant ERR_VERIFICATION_FAILED
  (err u1)
)
(define-constant ERR_FAILED_TO_PARSE_TX
  (err u2)
)
(define-constant ERR_INVALID_ID
  (err u3)
)
(define-constant ERR_FORBIDDEN
  (err u4)
)
(define-constant ERR_TX_VALUE_TOO_SMALL
  (err u5)
)
(define-constant ERR_TX_NOT_FOR_RECEIVER
  (err u6)
)
(define-constant ERR_ALREADY_DONE
  (err u7)
)
(define-constant ERR_NO_STX_RECEIVER
  (err u8)
)
(define-constant ERR_BTC_TX_ALREADY_USED
  (err u9)
)
(define-constant ERR_NATIVE_FAILURE
  (err u99)
)
    ...
    (match (contract-call?
      ...
      (begin
        ...
        (match (get out
          ...
          out
          (if (>= (get value out) (get sats swap))
            (begin
              (map-set swaps id (merge swap { done: true }))
              (map-set submitted-btc-txs result id)
              (as-contract
                (stx-transfer? (get amount swap) tx-sender
...
(define-public (submit-swap-segwit
  ...
  (let (
    (swap (unwrap! (map-get? swaps id) ERR_INVALID_ID))
    (tx-buff
      (contract-call? .clarity-bitcoin-helper-wtx concat-wtx wtx witness-data)
    )
  )
    (match (contract-call?
      SP2PABAF9FTAJYNFZH93XENAJ8FVY99RRM50D2JG9.clarity-bitcoin-lib-v5
